...
def make_retrieval_result(chunk: RetrievedKnowledgeChunk) -> RetrievalResult:
...
def test_policy_evidence_builds_from_retrieved_chunk() -> None:
    chunk = RetrievedKnowledgeChunk(
        chunk_id="chunk-1",
        ...
        content_hash="a" * 64,
        distance=0.2,
        metadata={"cross_encoder_score": "9.1"},
    )

    evidence = PolicyEvidence.from_retrieved_chunk(chunk, index=1)

    assert evidence.evidence_id == "E1"
    assert evidence.content == "Policy evidence content."
    assert evidence.citation.source_id == "eylf-v2"
    assert evidence.relevance_distance == 0.2
    assert evidence.metadata == {"cross_encoder_score": "9.1"}


def test_policy_rag_answered_requires_answer() -> None:
    chunk = RetrievedKnowledgeChunk(
        chunk_id="chunk-1",
        content="Policy evidence content.",
        citation=CitationMetadata(
    ...
    retrieval = make_retrieval_result(chunk)

    with pytest.raises(ValidationError):
        PolicyRAGResult(
            status=PolicyRAGStatus.ANSWERED,
            question="What does EYLF say?",
            retrieval=retrieval,
        )

    result = PolicyRAGResult(
        status=PolicyRAGStatus.ANSWERED,
        question="What does EYLF say?",
        answer="Use play-based learning with intentionality.",
        evidence=[PolicyEvidence.from_retrieved_chunk(chunk, index=1)],
...
def test_policy_rag_clarification_requires_question() -> None:
    chunk = RetrievedKnowledgeChunk(
        chunk_id="chunk-1",
        content="Policy evidence content.",
        citation=CitationMetadata(
    ...
    result = PolicyRAGResult(
        status=PolicyRAGStatus.NEEDS_CLARIFICATION,
        question="What policy applies?",
        clarification_question="Which policy area should I focus on?",
        retrieval=retrieval,
    )

    assert result.clarification_question == "Which policy area should I focus on?"


def test_policy_rag_refusal_or_conflict_requires_reason() -> None:
    chunk = RetrievedKnowledgeChunk(
        chunk_id="chunk-1",
        content="Policy evidence content.",
        citation=CitationMetadata(
    ...
    with pytest.raises(ValidationError):
        PolicyRAGResult(
            status=PolicyRAGStatus.REFUSED,
            question="Can I diagnose this?",
            retrieval=retrieval,
        )
    ...
    result = PolicyRAGResult(
        ...
        refusal_reason="Retrieved evidence contains conflicting versions.",
        retrieval=retrieval,
    )
